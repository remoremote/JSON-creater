...
async function scrapePage(link) {
    ...
    const addressLink = $(".btn.config-bg-primary.form-control.config-border-radius.contact-map-link").attr('href');
    if (addressLink) {
    ...
    details.descriptionMarkdown = turndownService.turndown(details.descriptionHTML); // Convert HTML to Markdown
